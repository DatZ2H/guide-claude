...
- Với mỗi file trong `guide/base/*.md`, `guide/doc/*.md`, `guide/dev/*.md`, `guide/reference/*.md` (BỎ QUA .bak):
...
- Grep các thuật ngữ quan trọng across tất cả guide/ modules, check dùng nhất quán:
...
**Modules:** {N} scanned (base/{X} + doc/{Y} + dev/{Z} + reference/{W})
