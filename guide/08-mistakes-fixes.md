...
**Fix:** Dùng Projects. Upload tài liệu dự án vào Project Knowledge. Viết Project Instructions mô tả context. Xem Module 02.
...
**Fix:** Dùng Handover (Module 04, mục 4.5) hoặc Projects (Module 02).
...
**Fix:** Dùng Context Refresh khi thấy dấu hiệu drift. Tạo conversation mới với Handover khi vượt 50 messages. Chi tiết: Module 04.
...
**Fix:** Đây là đặc tính kỹ thuật của cách language models tạo text, không phải lỗi do prompt. Workaround chính: tách output thành nhiều prompts, mỗi prompt chỉ viết 1-2 sections, dùng Pattern Outline-First (tạo outline trước → duyệt → viết từng section). Chi tiết cơ chế và 3 workaround patterns: Module 04 mục 4.8.
...
**Xem thêm:** Module 10 mục 10.7 (An toàn Cowork), Module 09 mục 9.6 (Review giữa các bước), Module 03 mục 3.5 (Task Decomposition — prevention ở cấp độ planning)
...
|--------|----------------|-----------------|
| Kết quả không liên quan | Thêm context và constraints | 03 (Prompt Engineering) |
| Claude "quên" giữa conversation | Context Refresh hoặc Handover | 04 (Context Management) |
| Hallucination | Thêm `<quality_requirements>`, yêu cầu self-check | 03, 09 |
| Output sai format | Cung cấp template hoặc ví dụ trong prompt | 03 (Nguyên tắc 2: Use Examples) |
| Output dài bị sơ sài ở cuối | Tách thành nhiều prompts, dùng Outline-First | 04 (mục 4.8), 03 (mục 3.5) |
| Lỗi lan truyền qua nhiều bước | Validation checkpoint, backup trước mỗi step | 08 (Nhóm 6) |
| Mỗi chat phải giải thích lại context | Tạo Project với Instructions | 02 (Setup) |
| Không biết dùng tool nào | Tra cứu bảng tools | 06 (Tools & Features) |
| Cần template có sẵn | Tra cứu Template Library | 07 (Templates) |
| Output quá chung | Thêm role, audience, ví dụ mong muốn | 03 (Nguyên tắc 1, 6) |
...
- Module 09: Evaluation Framework -- cách đánh giá chất lượng output
- Module 03: Prompt Engineering -- hiểu sâu nguyên tắc để tránh lỗi từ đầu
