...
| Claude Sonnet 4.5 | 200,000 tokens (chuẩn), 1M tokens (beta) | 500–700 trang / 2,500–3,500 trang |
...
> **Lý do giảm từ 4 → 2 files:** `handoff.md` và `todo.md` overlap với `session-state.md` (cùng chứa "đã làm gì / còn gì"). `context-map.md` dễ stale và có thể thay bằng cách Claude đọc trực tiếp file system. `TodoWrite` built-in của Cowork đã handle task tracking. Kết quả: token cost đầu session giảm, ít file cần maintain hơn.

| File | Ai ghi | Khi nào ghi | Khi nào đọc |
|------|--------|-------------|-------------|
| `session-state.md` | Claude (đầu/cuối session) hoặc bạn | Đầu và cuối mỗi session | Đầu session mới để nắm bối cảnh |
| `decisions-log.md` | Claude (khi có quyết định) | Mỗi khi chốt decision quan trọng | Khi cần nhớ lại lý do quyết định |

**So sánh với handover-note.md đơn lẻ (Module 10):**

| Aspect | handover-note.md (10.10) | `_memory/` folder (4.7) |
|--------|------------------------|------------------------|
| Scope | 1 file, 1 session | 2 files, nhiều sessions |
| Lịch sử quyết định | Không — chỉ snapshot cuối | Có — `decisions-log.md` tích lũy |
| Trạng thái hiện tại | Tóm tắt trong 1 file | `session-state.md` có cấu trúc |
| Task tracking | Trong "Còn lại" | `session-state.md` + TodoWrite built-in |
| Effort | Thấp | Thấp-Trung bình (2 files, dễ maintain) |
| Khi nào dùng | Task ngắn, 1-2 sessions | Dự án dài, nhiều sessions |

> **Lưu ý:** `_memory/` folder là best practice phổ biến trong cộng đồng AI agents, không phải tính năng chính thức của Anthropic. Hiệu quả phụ thuộc vào việc duy trì kỷ luật ghi file.

**Xem thêm:**
- Module 05 (Recipe 5.11) — Quy trình Hybrid Workflow sử dụng `_memory/` folder
- Module 10 (mục 10.8) — Cấu hình Cowork cho external memory
...
- `_memory/` — session-state, decisions-log
...
- Cây thư mục thực tế
- Bảng quyết định gần nhất (import từ `_memory/decisions-log.md`)
...
| Team member không hiểu model | Thêm "Knowledge Model" block vào Custom Instructions — xem `_memory/session-state.md` |
