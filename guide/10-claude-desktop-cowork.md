...
**13 Enterprise Connectors mới (24/02/2026):** Google Workspace (Calendar, Drive, Gmail), DocuSign, Apollo, Clay, Outreach, Similarweb, MSCI, LegalZoom, FactSet, WordPress, Harvey và các partners.
...
Agent Teams cho phép nhiều Claude agents phối hợp trong cùng một task — 1 orchestrator điều phối, các sub-agents xử lý task chuyên biệt. Phù hợp task phức tạp cần xử lý song song (ví dụ: review 20 files cùng lúc). Kiến trúc:
